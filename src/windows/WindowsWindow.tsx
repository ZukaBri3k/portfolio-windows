...
export function WindowsWindow() {
  ...
  const { menuState, setMenuState } = useContext(MenusContext);
  ...
  function close() {
    anime({
      ...
      complete: () => {
        setMenuState({
          ...menuState,
          windowsMenu: { ...menuState?.windowsMenu, open: false },
        });
      },
    });
  }
  ...
  return createPortal(
    <div className="w-full h-full">
      <div
        ref={modalRef}
        className="z-20 absolute right-1/2 bottom-[-600px] translate-x-1/2 translate-y-1/2 lg:w-5/12 md:w-4/5 h-5/6 rounded-2xl bg-slate-800/95 backdrop-blur-xl flex justify-center items-center flex-col border-[0.5px] border-slate-600"
      >
        <div className="flex items-center w-11/12 h-fit gap-5 bg-slate-900 p-2 m-10 rounded-3xl border-[0.5px] border-slate-700">
